...
class ParsingMethodContent: NSObject {
    // 查找method中使用过的类
    class func parsing(method: Method, file: File) -> [Object] {
        var results = [Object]()
        for tk in method.tokens {
            guard let obj = file.importObjects[tk] else {
                continue
            }
            if file.usedObjects[tk] == nil {
                // 记录使用过的类
    ...
    // -----------处理用过的方法------------
    class func parsing(contentArr: [String], inMethod: Method) -> Method {
        var mtdIn = inMethod
        ...
        for var tk in contentArr {
            ...
            if tk == Sb.bktL {
                ...
                if prm.name != "" {
                    ...
                    let pnameId = (uMtdDic[psBrcStep]?.pnameId.appending("\(prm.name):"))!
                    uMtdDic[psBrcStep]?.pnameId = pnameId
                }
            } else if tk == Sb.qM {
                psCdtTf = true
            } else {
