...
class ParsingBase: NSObject {
    ...
    class func createOCTokens(conent: String) -> [String] {
        ...
        while !scanner.isAtEnd {
            ...
            for operater in operaters {
                if let _ = scanner.scanString(operater) {
    ...
    class func dislodgeAnnotaion(content: String) -> String {
        ...
        var newStr = ""
        newStr = regexLine.stringByReplacingMatches(in: content, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSRange(location: 0, length: content.count), withTemplate: Sb.space)
        newStr = regexBlock.stringByReplacingMatches(in: newStr, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSRange(location: 0, length: newStr.count), withTemplate: Sb.space)
        return newStr
    }
    // 一行内清理注释
    class func dislodgeAnnotaionInOneLine(content: String) -> String {
        ...
        let regexLine = try! NSRegularExpression(pattern: annotationLinePattern, options: NSRegularExpression.Options(rawValue: 0))
        var newStr = ""
        newStr = regexLine.stringByReplacingMatches(in: content, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSRange(location: 0, length: content.count), withTemplate: "")
        return newStr
    }
}
